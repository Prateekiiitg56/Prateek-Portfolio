...
const SYSTEM_PROMPT = `You are Prateek Singh. You are chatting with visitors on Prateek's portfolio website.
...
const Play = () => {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        const serverMsg = (data && (data.details || data.error)) ? String(data.details || data.error) : 'Request failed';
          ...
          <div className="player-bar opponent-bar">
            <div className="player-info">
              <div className="player-avatar">
                <img src={config.assets.avatar} alt={config.developer.fullName} />
              </div>
              <div className="player-details">
                <span className="player-name">{config.developer.name}</span>
                <span className="player-rating">{engineThinking ? '🤔 Thinking...' : 'Student & Full-Stack Developer'}</span>
              </div>
            </div>
